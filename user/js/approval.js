...
(function(){
    var user = {
        ...
        renderPage:function(){//初始化第一页
            ...
            this.extendFun();
            //this.waitTable.loadData('/UserManagerAction!findAllRegister.action','user');
        ...
        agreeFun:function(data,mark){
            ...
            switch (mark){
                case '1'://第一页同意
                    $.post('js/status.json',data,function(res){// /UserManagerAction!passBatchRegisterUsers.action
                        if(res.code == 0){
                            TIP('操作成功！','success',2000);
                            _this.waitTable.loadData('js/table1.json','user');// /UserManagerAction!findAllRegister.action
                    ...
                    break;
                case '2'://第二页同意
                    $.post('/UserManagerAction!passBatchRegisterUsers.action',data,function(res){
                        if(res.code == 0){
                            TIP('操作成功！','success',2000);
        ...
        refuseFun:function(data){
            var _this = this;
            $.post('js/status.json',data,function(res){// /UserManagerAction!deleteBatchRegiUsers.action
                if(res.code == 0){
                    TIP('操作成功！','success',2000);
                    _this.waitTable.loadData('js/table1.json','user');///UserManagerAction!findAllRegister.action
                    _this.refuseTable.loadData('js/table1.json','user',function(){
